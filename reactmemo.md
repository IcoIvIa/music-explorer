...
#javascriptの関数たち
        ...
        ##三項演算子（JSでもある
          ...
          もし左側が空（null/undefined）だったら、そこで処理を止めて undefined を返す
          （nullが返されるとクラッシュする処理があるので、）

          便利な組み合わせ技： ?. と ||
// 「データがあればその名前を、なければ '不明' と表示してね」
const name = artist?.name || "不明";

// 「データがあれば map して、なければ空の配列 [] として扱ってね」
