...
２　最下層を追加する際のアルゴリズムを変更。
現在は最下層のアーティストをクリックすると、そのクリックされたアーティストにたいしてgetSimilarArtists()        → Last.fm API
│   └ setLayers()        
が起動するが、これをやめる。　適当な場所にDIGボタンを設置して。DIDぼたんをクリックするとDetailPanelに表示されてアーティストにたいして
getSimilarArtists()        → Last.fm API
│   └ setLayers() 
が起動するようにする。
（このDIGボタンをクリックについて、もっと直感的に操作できるアイデアがあれば教えてほしい）

3 ロジックのUIの分離　を進めていく。　冗長なコードを整理。
